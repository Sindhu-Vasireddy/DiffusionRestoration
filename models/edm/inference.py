...
from models.edm.diffusion_stochastic_sampler import KarrasStochasticSampler
from models.edm.diffusion_constrained_sampler import KarrasStochasticConstrainedSampler
...
class DiffusionInference:
    ...
    def rollout(
        ...
        sampler = KarrasStochasticSampler(
        ...
        if self.noise_shape[0] > 1:
            predictions = [x_current.unsqueeze(0).cpu()]
        else:
            predictions = [x_current.cpu()]
        ...
        if  config.show_rollout_progress:   
            ...
            num_steps = tqdm(
                num_steps,
                total=len(num_steps[1:]),
                desc=f"Rollout step {i}",
            )
        ...
        if config.to_physical:
            self.predictions = self.transforms.apply_inverse_transforms(
                self.predictions
            )
