...
class Conv2d(torch.nn.Module):
    def __init__(
        ...
        self.circular_height = circular_height 
        self.circular_width = circular_width 
    ...
    def apply_circular_padding(self, x, pad_w, pad_h):
        """Applies circular padding along width and optionally height."""

        if pad_w > 0:
            if self.circular_width:
                x = F.pad(x, (pad_w, pad_w, 0, 0), mode='circular')  
            else:
                x = F.pad(x, (pad_w, pad_w, 0, 0), mode='constant', value=0)  
    
        if pad_h > 0:
            if self.circular_height:
    ...
    def forward(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            x = self.apply_circular_padding(x, max(f_pad - w_pad, 0), max(f_pad - w_pad, 0))
            x = F.conv_transpose2d(
                x,
                f.mul(4).tile([self.in_channels, 1, 1, 1]),
                groups=self.in_channels,
                stride=2,
                padding=0,  # No implicit zero padding
            )
            x = self.apply_circular_padding(x, max(w_pad - f_pad, 0), max(w_pad - f_pad, 0))
            x = F.conv2d(x, w, padding=0)
        elif self.fused_resample and self.down and w is not None:
            x = self.apply_circular_padding(x, f_pad, f_pad)
            x = F.conv2d(x, w, padding=0)
            x = F.conv2d(
                x, f.tile([self.out_channels, 1, 1, 1]), groups=self.out_channels, stride=2, padding=0
            )
        else:
            if self.up:
                x = self.apply_circular_padding(x, f_pad, f_pad)
            ...
            if w is not None:
                x = self.apply_circular_padding(x, w_pad, w_pad)
                x = F.conv2d(x, w, padding=0)
...
class UNetBlock(torch.nn.Module):
    def __init__(
        ...
        self.conv0 = Conv2d(
            ...
            circular_height=circular_height,  
...
class PeriodicSongUNet(torch.nn.Module):
    ...
    def __init__(
        ...
        circular_height=False,  # New: Enable circular padding in height
        circular_width=False,  # New: Enable circular padding in height
        use_grid_embedding=False,  # New: Enable circular padding in height
    ):
        ...
        self.use_grid_embedding = use_grid_embedding
        if use_grid_embedding:
            self.grid_embedding = torch.cat(GridEmbedding2D(1).grid([180,360], "cuda", torch.float32), dim=1)
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f"{res}x{res}_conv"] = Conv2d(
                    in_channels=cin, out_channels=cout, kernel=3,
                    circular_width=circular_width, circular_height=circular_height, **init
                )
            else:
                self.enc[f"{res}x{res}_down"] = UNetBlock(
                    in_channels=cout, out_channels=cout, down=True, 
                    circular_width=circular_width, circular_height=circular_height, **block_kwargs
                )
                if encoder_type == "skip":
                    ...
                    self.enc[f"{res}x{res}_aux_skip"] = Conv2d(
                        in_channels=caux, out_channels=cout, kernel=1,
                        circular_width=circular_width, circular_height=circular_height, **init
                    )
            ...
            for idx in range(num_blocks):
                ...
                self.enc[f"{res}x{res}_block{idx}"] = UNetBlock(
                    in_channels=cin, out_channels=cout, attention=attn,
                    circular_width=circular_width, circular_height=circular_height, **block_kwargs
                )
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                self.dec[f"{res}x{res}_in0"] = UNetBlock(
                    in_channels=cout, out_channels=cout, attention=True,
                    circular_width=circular_width, circular_height=circular_height, **block_kwargs
                )
                ...
                self.dec[f"{res}x{res}_up"] = UNetBlock(
                    in_channels=cout, out_channels=cout, up=True,
                    circular_width=circular_width, circular_height=circular_height, **block_kwargs
                )
            ...
            if decoder_type == "skip" or level == 0:
                ...
                self.dec[f"{res}x{res}_aux_conv"] = Conv2d(
                    in_channels=cout, out_channels=out_channels, kernel=3,
                      circular_width=circular_width, circular_height=circular_height, **init_zero
                )
    ...
    def forward(self, x, noise_labels, conditioning=None, class_labels=None, augment_labels=None):
        ...
        if self.use_grid_embedding:
            x = torch.cat((x, self.grid_embedding.repeat(x.shape[0],1,1,1)), 1)
