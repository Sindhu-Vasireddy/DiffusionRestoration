...
def load(
    ...
    if multi_files:
        ...
        ds = xr.open_mfdataset(
            file_name, chunks=chunks, combine=combine, concat_dim=concat_dim
        )
        if is_grib:
            ds = xr.open_mfdataset(
                file_name, chunks=chunks, combine="by_coords", engine="cfgrib"
            )
    else:
...
def crop_to_reference(dataset: xr.Dataset, ref_dataset: xr.Dataset) -> xr.Dataset:
    ...
    dataset = dataset.where(dataset.latitude == ref_dataset.latitude, drop=True).where(
        dataset.longitude == ref_dataset.longitude, drop=True
    )
...
def select_times(
    dataset: xr.Dataset, reference_time: list
...
def remove_seasonality(data: xr.DataArray) -> xr.DataArray:
    climatology = data.groupby("time.month").mean("time")
    deseasonalized_data = data.groupby("time.month") - climatology
